...
class Handler{
    ...
    public function handle_webhook($request){
        ...
        error_log('[Pay-in-3]Webhook recieved: ' . $body);
    ...
    private function verify_signature($request){
        ...
        if(empty($signature_header[0])){
            error_log('[Pay-in-3] Webhook: Missing signature header.');
            return false;
        }
    ...
    private function is_duplicate_event($request){
        ...
        if(empty($idempotency_header[0])){
            error_log('[Pay-in-3] Webhook: Missing idempotency header. Processing, but risk of replay present.');
            return false;
        }
        ...
        if(get_transient($idempotency_key)){
            error_log('[Pay-in-3] Webhook: Duplicate event blocked. Key: ' . $idempotency_key);
            return true;
        }
